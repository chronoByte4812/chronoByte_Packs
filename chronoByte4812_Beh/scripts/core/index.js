...
world.beforeEvents.chatSend.subscribe((data) => {
    ...
    if (command === undefined) return sender.sendMessage(`§cThe command §f${cmd}§c was not found`);
    if (command.for_staff === true && sender.hasTag(config.staffTag) === false) return sender.sendMessage(`§cThe command §f${cmd}§c is for staff only`);
